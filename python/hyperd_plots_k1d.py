...
def HyperDPanelCanvas(name, n_xbins, n_ybins, x_size=1000, y_size=750):
...
def MakeDataMCStackLists(hist_dict, stack_name,binvol_list,sample="QElike",multipliers=[]):
    n_ybins = hist_dict["data"].GetNbinsY()
    stack_list = []
    data_list = []
    ...
    for i in range(n_ybins):
        ...
        data = TH1D()
        for cat in order:
            h2d = hist_dict[cat].Clone()
            tmp_proj_name = "proj_"+cat+"_"+str(i)
            proj = h2d.ProjectionX(tmp_proj_name, i+1, i+1)
            proj = MakeHistPretty(proj, cat,binvol_list[i])
...
def GetBinRangeStringList(h2d_template, varname):
    n_ybins = h2d_template.GetNbinsY()
    out_string_list = []
    for bin in range(1,n_ybins+1):
        bin_low = h2d_template.GetYaxis().GetBinLowEdge(bin)
        bin_hi = h2d_template.GetYaxis().GetBinUpEdge(bin)
...
def PlotDataMCOnGrid(grid_canvas, title, data_list, stack_list, multipliers = None, plotmax = None, bin_range_string_list = None):
    ...
    grid_canvas.SetXTitle("recoil (GeV)")
    grid_canvas.SetTitleSize(20.0)
    grid_canvas.ResetPads()
    grid_canvas.Draw()

def GetHyperDHistDict(rfile):
...
def MakeHistPretty(hist,category,binvol):
    hist.Scale(1.*binvol, "width")
...
def main():
    ...
    if len(sys.argv) == 1:
        print ("python hyperd_plots.py <rootfile> <option>")
    flag = "types_"
    filename = sys.argv[1]
    if len(sys.argv)> 2:
        flag = "tuned_type_"
    ...
    dirname = filename.replace(".root","_HyperDplots")
    ...
    for a_sample in hist_dict.keys():
        ...
        for b_var in hist_dict[a_sample].keys():
            ...
            myHyperDim = PlotUtils.HyperDimLinearizer(global_bins_dict[b_var], 1)
            # print("myHyperDim.GetValues(1)=",myHyperDim.GetValues(1))

            # test_hyperdim = PlotUtils.HyperDimLinearizer([[1.,2.4,3.5,4.7],[1.,2.4,3.5,4.7],[1.,2.4,3.5,4.7]], 1)
            # test_val = 0
            # print("test_hyperdim.GetValues(",test_val,")=",test_hyperdim.GetValues(test_val))
            h2D_dict = {}
            total_max = None
            ...
            for z_bin in h2D_dict.keys():
                ...
                canvas_name = "%s_%s_%0.2d"%(a_sample,b_var,z_bin)
                canvas_title = "%s %s"%(a_sample,b_var)
                gc2 = HyperDPanelCanvas(canvas_name, n_xbins, n_ybins)
                ...
                data_list, tmp_stack_list = MakeDataMCStackLists(h2D_dict[z_bin],stack_name,binvol_list,a_sample)
                ...
                ofilename = str(dirname+"/"+str(b_var)+"/"+canvas_name+"_FinalStates_"+str("{0:0=2d}".format(z_bin))+".pdf")
                gc2.Print(ofilename)
