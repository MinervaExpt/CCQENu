...
colorcode = {"NA":colors[0], "2p2hrpa":colors[3], "rpapiontune":colors[4], "2p2hpiontune":colors[5],"2p2h":colors[6], "CV":colors[10], "CV2":colors[5],"piontune":colors[1],"rpa":colors[9],"gen2p2h":colors[6]}
linecode = {"NA":1, "2p2hrpa":10, "rpapiontune":10, "2p2hpiontune":10,"2p2h":1, "CV":1, "CV2":1,"piontune":10,"rpa":1,"gen2p2h":1}
linewidth = {"NA":3, "2p2hrpa":3, "rpapiontune":3, "2p2hpiontune":3,"2p2h":3, "CV":4, "CV2":4,"piontune":3,"rpa":3,"gen2p2h":3}
...
_xdtot.GetYaxis().SetLabelSize(0.05);
_xdtot.GetYaxis().SetTitleSize(0.05);
_xdtot.GetYaxis().SetTitleOffset(1.2);

_xdtot.GetXaxis().SetLabelSize(0.05);
_xdtot.GetXaxis().SetTitleSize(0.06);
_xdtot.GetXaxis().SetTitleOffset(1.1);
...
if DRAWRATIO:
    ...
    xproj_titley = "#sigma(E_{#nu})(#times 10^{-39}cm^{2}/nucleon)"
xproj_titlex = "E_{#nu} (GeV)"


xd.GetYaxis().SetTitle(xproj_titley)
xd.GetXaxis().SetTitle(xproj_titlex)
xdtot.GetYaxis().SetTitle(xproj_titley)
xdtot.GetXaxis().SetTitle(xproj_titlex)
_xdtot.GetYaxis().SetTitle(xproj_titley)
_xdtot.GetXaxis().SetTitle(xproj_titlex)
...
if DRAWRATIO:
    ...
    xd.SetMaximum(11.0)
    xd.SetMinimum(0.0)
    xdtot.SetMaximum(11.0)
    xdtot.SetMinimum(0.0)
    _xdtot.SetMaximum(11.0)
    _xdtot.SetMinimum(0.0)
xd.Draw("PE")
xdS.Draw("E1 SAME")

# draw the total cross section
if(not Fiducial):
    if not DRAWRATIO:
        _xmctot.SetLineColor(ROOT.kGreen+3)
        _xmctot.SetLineWidth(4)
        #_xmctot.SetLineStyle(5)
        _xmcmnv2tot.SetLineColor(ROOT.kGreen+3)
        _xmcmnv2tot.SetLineStyle(2)
        _xmcmnv2tot.SetLineWidth(4)
...
#ymcgibu.Print("ALL")
leg = ROOT.TLegend(0.3,0.175,0.7,0.4)
leg.SetFillStyle(0)
leg.SetBorderSize(0)
...
if not Fiducial:
 SFiducial = "Full"
if DRAWRATIO:
    canvas.Print("xsectionmodels_variations_ProjectionX_ratio_%s.png"%SFiducial)
    canvas.Print("xsectionmodels_variations_ProjectionX_ratio_%s.eps"%SFiducial)
    canvas.Print("xsectionmodels_variations_ProjectionX_ratio_%s.C"%SFiducial)
else:
    canvas.Print("xsectionmodels_variations_ProjectionX_%s.png"%SFiducial)
    canvas.Print("xsectionmodels_variations_ProjectionX_%s.eps"%SFiducial)
    canvas.Print("xsectionmodels_variations_ProjectionX_%s.C"%SFiducial)
#sys.exit()
