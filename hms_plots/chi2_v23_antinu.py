...
Ratio = False # this is just for plotting
...
if var in ["q2"]:
  ...
  if not NOREBIN: thefile = thefile.replace(".root","_rebinned.root")
if var in ["enu","enuQE"]:
...
for model in models:
    ...
    if "G18" in model:
        if var == "q2" and not NOREBIN:
            thegeniefile = "xsection_%s_50Mcombined_RHC_rebinned.root"%model
        else:
            thegeniefile = "xsection_%s_50Mcombined_RHC.root"%model
...
#        mcbasehist = "MMECCQEGENIEOOBnu/muonptVSmupzall"
   ...
   #   twoD = False
    ...
    if mcfile == None:
      ...
      continue
    mchist[model] = MnvH2D()
    print (" try to read in model ", model,mcbasehist,geniehist)
    if "G18" not in model:  # is it the special genie?
        ...
        if (var != "q2"):
            ...
            mchist[model] = MnvH1D(mcfile[model].Get(geniehist).Clone())
            proj = ""
