...
python $APP/NEWMAT/CCQENu/utilities/SubmitJobsToGrid_MAT.py --stage=CCQEMAT --outdir=$SCRATCH/test --basedir=$APP/NEWMAT --rundir=CCQENu/make_hists --playlist=minervame5A --tag=test --mail --prescale=1000 --config=$CCQEMAT/testme --exe=CCQENu/make_hists/sidebands_v2 --setup=CCQENu/utilities/setup_batch_mat.sh --tmpdir=$BLUE/tmp #--debug --notimestamp
