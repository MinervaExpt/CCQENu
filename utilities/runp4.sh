...
python $WHEREIPUTMYCODE/CCQENu/utilities/SubmitJobsToGrid_MAT.py --stage=CCQEMAT --outdir=$SCRATCH/test \
 --basedir=$WHEREIPUTMYCODE --rundir=CCQENu/make_hists --playlist=minervame5A --model=MnvTunev2 --tag=runp4 \
 --mail --prescale=1 --config=p4_run --exe=sidebands_v2 --sample=QElike --setup=CCQENu/utilities/setup_batch_mat9.sh \
