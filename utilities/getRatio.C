...
int main(const int argc, const char *argv[]){
  ...
  std::cout << std::endl;
    ...
    for(TObject* keyAsObj1 : *file1->GetListOfKeys()) {
    	auto key1 = dynamic_cast<TKey*>(keyAsObj1);
        std::string name1 = key1->GetName();
        std::string classname1 = key1->GetClassName();       
        if( classname1 == "PlotUtils::MnvH1D" ) {
        
				  for(TObject* keyAsObj2 : *file2->GetListOfKeys()) {
					auto key2 = dynamic_cast<TKey*>(keyAsObj2);
					std::string name2 = key2->GetName();
					std::string classname2 = key2->GetClassName();
					
					if(name2 == name1 && classname2 == "PlotUtils::MnvH1D") {
