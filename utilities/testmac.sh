...
python $APP/TOPMAT/CCQENu/utilities/SubmitJobsToGrid_MAT.py --stage=CCQEMAT --outdir=$SCRATCH/test --basedir=$APP/TOPMAT --rundir=CCQENu/make_hists --playlist=minervame5A --tag=test --mail --prescale=1000 --exe=sidebands_v2 --config=./hms/testmac --setup=CCQENu/utilities/setup_batch_osx.sh --tardir=$HOME/tmp/test --tmpdir=$HOME/tmp/ #--debug --notimestamp
