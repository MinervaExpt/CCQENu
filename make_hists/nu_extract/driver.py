...
if not prefit:
    ...
    allconfigs["Fit"] = commentjson.load(inputconfigfile)
...
print (commentjson.dumps(allconfigs["Fit"], indent=4))
...
#AnalyzeVariables = allconfigs["Fit"]["Variables"]
categoryMap = {}
signalMap = {}
count = 0
if rescale:
...
if not hasbkgsub:
    print (" no fitconfig in the input file, I need output of the fitter to proceed:")
    sys.exit(1)
...
if (not singlesample): 
    ...
    SampleRequest = allconfigs["Fit"]["Samples"]
print (SampleRequest)
...
# this is the old read in the POT.  They are also in a vector which Amit liked having
...
for k in f.GetListOfKeys():
    ...
    #print ("parsed key",parsekey)

    if category not in allconfigs["Fit"]["Categories"]:
        print ("category ", category, " not in config categories, skipping")
        continue
    if variable not in allconfigs["Fit"]["Variables"]:
        print ("variable ", variable, " not in config variables, skipping")
        continue
...
noscale = ["data","all","bkg","bkgsub","qelike"]
