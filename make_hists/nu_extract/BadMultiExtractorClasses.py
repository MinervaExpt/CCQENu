...
import commentjson
from SyncBands import SyncBands, CopyBands
from UnfoldUtils import MnvUnfold, MnvResponse
from RebinFlux import GetFluxFlat

DEBUG=True
PLOTS=True
POPFITPARAMETERS=False # have to do this for now to keep unfolding happy about match between data and mc. HMS 3-7-2026
...
def addentry(thing,one,two,three,four,value):
    ...
    if thing[one][two][three] == {}:
        ...
        if len(names) > 0:
            SyncBands(value)
        if DEBUG: print ("added",thing[one][two][three][four].GetName(),one,two,three,four,value.GetName(),len(names))
    else:
...
class CrossSectionExtractor:
    ...
    def background(self):
        ...
        for sample in self.allconfigs["Cross"]["Samples"]:
            for variable in self.allconfigs["Cross"]["Variables"]:
                ...
                new_name = data_hist.GetName()+"_"+input_stage
                
                bkgsub_hist = data_hist.Clone(new_name)
                #bkgsub_hist.AddMissingErrorBandsAndFillWithCV(sig_hist)
                
                CopyBands(bkgsub_hist,sig_hist)
                CopyBands(bkgsub_hist,bkg_hist)
    ...
    def unfold(self,iterations=4):
        ...
        selected_type = self.grabber.allconfigs["Cross"]["Selected_Type"]
        for sample in self.allconfigs["Cross"]["Samples"]:
            for variable in self.allconfigs["Cross"]["Variables"]:
                logscale = self.allconfigs["Cross"]["Scales"][variable]
                data_cat = self.allconfigs["Cross"]["Data_Cat"]
                ...
                if DEBUG: print (f" unfold {sample} {data_cat} {variable} {input_stage}")
                data_hist = self.hists1D[sample][data_cat][variable][input_stage]
                ...
                CopyBands(data_hist,true_sel_hist)
                CopyBands(data_hist,migration)
                unfolded_hist= MnvH1D()
                unfolded_hist= MnvH1D((true_sel_hist).GetCVHistoWithStatError())
                #unfolded_hist = data_hist.Clone(unfolded_name)
                #unfolded_hist.Reset()
                print("copy bins from data to unfolded")
                CopyBands(data_hist,unfolded_hist)
    ...
    def efficiency(self,thesample):
        ...
        selected_type = self.grabber.allconfigs["Cross"]["Selected_Type"]
        truth_type = self.grabber.allconfigs["Cross"]["Truth_Type"]
        for sample in self.grabber.hists1D.keys():
            ...
            for variable in self.allconfigs["Cross"]["Variables"]:
                ...
                CopyBands(unfolded_hist,true_sel_hist)
                CopyBands(unfolded_hist,true_all_hist)
                ...
                CopyBands(corrected_hist,efficiency_hist)
        ...
        for sample in self.grabber.hists1D.keys():
            ...
            for variable in self.allconfigs["Cross"]["Variables"]:
                ...
                if (not self.allconfigs["Cross"]["FluxNorm"][variable]): 
                    ...
                    CopyBands(sigma_hist,theflux_hist)
                    ...
                    CopyBands(sigmaMC_hist,theflux_hist)
...
class DataGrabber:
    ...
    def __init__(self, config, plotter):
        ...
        self.potscales={}
        self.DoPOTScale = config["POTScale"]
        for sample in self.samples:
            ...
            mcPOTprescaled = h_pot.GetBinContent(3)
            POTScale = dataPOT / mcPOTprescaled
            self.potscales[sample] = POTScale
            ...
            for category in self.allconfigs["Cross"]["Categories"]:
                for variable in self.allconfigs["Cross"]["Variables"]:
                    for data_type in self.datatypes1D[category]:
                        ...
                        if self.DoPOTScale and category != data_cat:
                            hist.Scale(self.potscales[sample])
                        addentry(self.hists1D, sample, category, variable, data_type, hist)
                        if DEBUG: print ("grabbed",self.hists1D[sample][category][variable][data_type].GetName(),sample,category,variable,data_type,len(hist.GetVertErrorBandNames()))
...
# make an MCTOT and plot if request
                    ...
                    for data_type in self.hists1D[sample][category][variable].keys():
                        if sampleindex == 0:
                            ...
                            if "all_truth" in data_type: 
                                ...
                                continue
                            CopyBands(self.hists1D[sample][category][variable][data_type],self.hists1D[newsample][category][variable][data_type])
    ...
    def print(self,types=None,option=""):
        for sample in self.hists1D.keys():
            for category in self.hists1D[sample].keys():
                for variable in self.hists1D[sample][category].keys():
                    print ("keys",self.hists1D[sample][category][variable].keys())
                    for data_type in self.hists1D[sample][category][variable].keys():
                        
                        if types is not None and data_type not in types: continue
                        print ("datatype",data_type,self.hists1D[sample][category]
                        [variable])
                        if data_type not in self.hists1D[sample][category][variable]: print("strange")
                        print ("type",type(self.hists1D[sample][category]
                        [variable]))
                        hist = MnvH1D()
                        hist = self.hists1D[sample][category]
                        [variable][data_type]
                        nbands = hist.GetVertErrorBandNames()
                        count = self.hist.Integral()
                        print (f"Totals: {sample} {category} {variable} {data_type} {count} {nbands}")
        pass
...
if __name__ == "__main__":
    ...
    grabber.grab()
    print ("------------ total mc ---------------")
    grabber.mctotal() # sum up mc if it is not already summed. 
    #grabber.print(["reconstructed"])
