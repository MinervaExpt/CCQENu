...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
   ...
   while ( (key = (TKey*)nextkey())) {
      ...
      TH1D *th1d = dynamic_cast<TH1D *>(obj);
      if (th1d) printf("TH1D object\n");
      ...
      TVector2 *tvector2 = dynamic_cast<TVector2 *>(obj);
    ...
    //   else if (obj->GetName() == std::string("main")) {
    //       std::cout << " write out named object " << obj->GetName() << std::endl;
    //       target->cd();
    //       TFile *nextsource = (TFile *)sourcelist->After(first_source);
    //       while (nextsource) {
    //           // make sure we are at the correct directory level by cd'ing to path
    ...
    //           TKey *key2 = (TKey *)gDirectory->GetListOfKeys()->FindObject(key->GetName());
    //           if (key2) {
    //               TNamed *h2 = dynamic_cast<TNamed* >(key2->ReadObj());
    //               TNamed->Write();
    //           }
    ...
    //   } 
          ...
          while (nextsource) {
              ...
              TKey *key2 = (TKey *)gDirectory->GetListOfKeys()->FindObject(key->GetName());
              if (key2) {
                  TH1D *h2 = dynamic_cast<TH1D *>(key2->ReadObj());
                  th1d->Add(h2);
                  delete h2;
              }
      ...
      } else if (obj->InheritsFrom("TVector2")) {
          // if ( obj->IsA()->InheritsFrom( TH1::Class() ) ) {
          // descendant of TH1 -> merge it
          ...
          while (nextsource) {
              ...
              TKey *key2 = (TKey *)gDirectory->GetListOfKeys()->FindObject(key->GetName());
              if (key2) {
                  TVector2 *h2 = dynamic_cast<TVector2 *>(key2->ReadObj());
                  *tvector2 += (*h2);
                  delete h2;
              }
          ...
          // object is of no type that we know or can handle just dump it
