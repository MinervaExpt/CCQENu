...
enum fit_type{
  ...
  private:
    ...
    int fFirstBin;
    int fLastBin;
    std::map<const std::string, int> fFirstBinMap;
    std::map<const std::string, int> fLastBinMap;
    int fNdim;
    fit_type fType;
    bool fDoFit;
  ...
  public:
   ...
   MultiScaleFactors(const std::map<const std::string, std::vector<TH1D*> > unfitHists, const std::map<const std::string, TH1D*> dataHist, const std::map<const std::string, bool> Include, const fit_type type, const std::map<const std::string, int> firstBinMap, const std::map<const std::string, int> lastBinMap);
