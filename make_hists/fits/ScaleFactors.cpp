...
ScaleFactors::ScaleFactors(const std::vector<TH1D*> fitHists, const std::vector<TH1D*> unfitHists, const TH1D* dataHist,
                           const int firstBin, const int lastBin) : IBaseFunctionMultiDimTempl<double>(), fFitHists(fitHists), fUnfitHists(unfitHists), fDataHist(dataHist), fFirstBin(firstBin), fLastBin(lastBin), fDoFit(true) {
    if (fFitHists.size() == 0) {
