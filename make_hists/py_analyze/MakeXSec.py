...
def GetCrossSection(sample, variable, basename,
    ...
    alltypes = [mctype,seltype,trutype,migtype,mrecotype,mtrutype]
    ...
    #         print ("new bkg",imcbkghist.GetEntries())
    #         imcbkghist.Print("ALL")

    #     else:
    #         imcbkghist = histsND[mctype]["qelikenot"]
    print ("getbkg",mctype,histsND[mctype].keys())
    ...
    print(" using " , imcbkghist.GetName() )
        ...
        #         sighists = []
        #         firstsig = sigcats[0]
        #         if atype not in histsND:
        #             print (" no ", atype, " in histsND",histsND.keys())
        #             continue
        #         newname = histsND[atype][firstsig].GetName().replace(firstsig,"sig")
        #         for cats in sigcats:
        #             print ("new sig",cats,newname)
        #             sighists.append(histsND[atype][cats])
        #             histsND[atype][cats].Print()
                
        #         histsND[atype]["sig"] = sumHists(sighists,newname=newname)
        #         SyncBands(histsND[atype]["sig"])
        #         print ("new sig",atype,histsND[atype]["sig"].GetEntries())
        #         histsND[atype]["sig"].Print("ALL")
        #     imcsighist = histsND[mctype]["sig"]
        # else:
        #     imcsighist = histsND[mctype]["qelike"]
    imcsighist = histsND[mctype]["sig"]
    imcsighist.Write()
    print(" using " , imcsighist.GetName() )
    #else:
    #    ibkgsubhist = histsND["fitted"]["bkgsub"]
        #isigsubhist = histsND["fitted"]["bkgsub"]
    ...
    # else: 
    if migtype not in responseND or "sig" not in responseND[migtype] :
        print (" Can't unfold this sample as no",migtype,sig,responseND[migtype])
        return
    if mrecotype not in histsND or "sig" not in histsND[mrecotype] :
        print (" Can't unfold this sample as no",mrecotype,sig,histsND[mrecotype])
        return
