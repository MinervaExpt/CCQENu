...
double CVUniverse::GetCosThetamu() const { return std::cos(GetThetamu());}
double CVUniverse::GetTrueCosThetamu() const { return std::cos(GetTrueThetamu()); }
...
// double CVUniverse::GetPrimaryProtonTrackEndX() const { return GetDouble((GetAnaToolName() + "_proton_endPointX").c_str()); }
// double CVUniverse::GetPrimaryProtonTrackEndY() const { return GetDouble((GetAnaToolName() + "_proton_endPointY").c_str()); }
// double CVUniverse::GetPrimaryProtonTrackEndZ() const { return GetDouble((GetAnaToolName() + "_proton_endPointZ").c_str()); }

ROOT::Math::XYZVector CVUniverse::GetPrimaryProtonTrackEnd() const {
    return ROOT::Math::XYZVector(CVUniverse::GetPrimaryProtonTrackEndX(), CVUniverse::GetPrimaryProtonTrackEndY(), CVUniverse::GetPrimaryProtonTrackEndZ());
}
...
double CVUniverse::GetPrimaryProtonPhi() const { return GetProtonPhi(0); }
// double CVUniverse::GetSecProtonPhi_1() const { return GetProtonPhi(1); }
// double CVUniverse::GetSecProtonPhi_2() const { return GetProtonPhi(2); }
// double CVUniverse::GetSecProtonPhi_3() const { return GetProtonPhi(3); }
// double CVUniverse::GetSecProtonPhi_4() const { return GetProtonPhi(4); }
...
int CVUniverse::SetRecoilBranch(NuConfig RecoilBranchConfig, bool print) {
...
double CVUniverse::GetVertexEnergyGeV() const {
    // Get's the energy inside the 100mm radius around the vertex, TODO: does this need calo tuning?
    return (GetDouble("recoil_energy_nonmuon_nonvtx0mm") - GetDouble("recoil_energy_nonmuon_nonvtx100mm"));
}
...
double CVUniverse::GetCalRecoilEnergy() const {
    ...
    return GetDouble(m_recoil_branch.c_str());
}
...
double CVUniverse::GetCalibRecoilEnergyGeV() const { 
    ...
    double recoil = 2 * GetRecoilEnergyGeV();
    // based off poly fit to EAvail with neutrons, intercept forced to zero
    return 3.1958 * recoil * recoil * recoil -2.3571 * recoil * recoil + 1.9268 * recoil;
...
double CVUniverse::GetEAvailDropCandsGeV() const {
...
double CVUniverse::GetEAvailFromBlobsGeV() const {
    if (GetNMADBlobs() == 0) return 0.0;
    double eavail = 0.0;
    std::vector<double> edeps = GetVec<double>((GetAnaToolName()+"_BlobTotalE").c_str());
    for (unsigned int i = 0; i < GetNMADBlobs(); i++) {
        eavail += edeps[i];
    }
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();

    // if (neutevent->GetNNeutCands() == 0) return eavail * MeVGeV;
    if (m_neutevent->GetNNeutCands() == 0) return eavail * MeVGeV;
    for (unsigned int i = 0; i < m_neutevent->GetNNeutCands(); i++) {
        // if (i == 1 && recoil > 0.3) break;
        if (i == m_maxNCands_neutron) break;  // defaults to 9999
        // eavail -= neutevent->GetNeutCand(i)->GetCandRecoEDep();
...
double CVUniverse::GetEAvailLeadingBlobGeV() const {
    ...
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    // double neutblobE = neutevent->GetNeutCand(0)->GetCandRecoEDep() * MeVGeV;
    double neutblobE = m_neutevent->GetNeutCand(0)->GetCandRecoEDep() * MeVGeV;
...
double CVUniverse::GetXVertex() const {
    std::vector<double> tmp = GetVec<double>("vtx");
    return tmp[0];
}

double CVUniverse::GetYVertex() const {
    std::vector<double> tmp = GetVec<double>("vtx");
    return tmp[1];
}
...
double CVUniverse::GetTrueXVertex() const {
    std::vector<double> tmp = GetVec<double>("mc_vtx");
    return tmp[0];
}

double CVUniverse::GetTrueYVertex() const {
    std::vector<double> tmp = GetVec<double>("mc_vtx");
    return tmp[1];
}
...
int CVUniverse::GetNNonVtxIsoBlobs() const {
    return GetInt("n_nonvtx_iso_blobs");
}

int CVUniverse::GetNNonVtxIsoBlobsAll() const {
    return GetInt("n_nonvtx_iso_blobs_all");
}

int CVUniverse::GetLeadingNonVtxIsoBlobPDG() const {
    return GetVecElem("nonvtx_iso_blobs_particle_pdg_in_prong",0);
}

int CVUniverse::GetLeadingNonVtxIsoBlobPrimaryPDG() const {
    return GetVecElem("nonvtx_iso_blobs_primary_particle_pdg_in_prong", 0);
}

int CVUniverse::GetTrueNBlobs() const {  // This is just messing around... -NHV 6/20/23
    int pi0_count = GetTrueNeutralPionCount();
    return pi0_count * 2.;
}
...
int CVUniverse::GetNMADBlobs() const {
    return GetInt((MinervaUniverse::GetTreeName() + "_BlobIs3D_sz").c_str());
}

std::vector<ROOT::Math::XYZVector> CVUniverse::GetBlobsBegPos() const {
    std::vector<ROOT::Math::XYZVector> positions = {};
    if (GetInt((GetAnaToolName() + "_BlobBegX_sz").c_str()) == 0)
        return positions;
    std::vector<double> BlobBegX = GetVec<double>((GetAnaToolName() + "_BlobBegX").c_str());
...
std::unique_ptr<NeutronMultiplicity::NeutEvent> CVUniverse::GetNeutEvent() const {
    ...
    if (!_is_neutron_config_set) {  // if the global config is not set
        std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = std::make_unique<NeutronMultiplicity::NeutEvent>(CVUniverse::GetNMADBlobs(),
                                                                                                                     vtx,
                                                                                                                     pmu,
                                                                                                                     CVUniverse::GetPrimaryProtonTrackEnd());  // default vals
    ...
    std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = std::make_unique<NeutronMultiplicity::NeutEvent>(CVUniverse::m_neutron_config,
                                                                                                                 CVUniverse::GetNMADBlobs(),
                                                                                                                 vtx,
                                                                                                                 pmu,
                                                                                                                 CVUniverse::GetPrimaryProtonTrackEnd());
    neutevent->SetReco(GetVec<int>((GetAnaToolName() + "_BlobID").c_str()),
...
int CVUniverse::GetNNeutCands() const {
    ...
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    // return neutevent->GetNNeutCands();
    return m_neutevent->GetNNeutCands();
}
...
int CVUniverse::GetTrueNNeutCands() const {
    ...
    // return neutevent->GetNTrueNeutCands();
    return m_neutevent->GetNTrueNeutCands();
}
...
int CVUniverse::GetAllBlobCandsNeut() const {
    ...
    for (int i = 0; i < GetNMADBlobs(); i++) {
        ...
        if (!m_neutevent->GetCandIsNeut(i)) return 0;
    }
    return 1;
}
...
void CVUniverse::PrintMADBlobs() const {
    if (GetNMADBlobs() != 0) {
        // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetTruthNeutEvent();
        // std::cout << "from cand      : ";
        // int ncands = neutevent->GetNCands();
        // for (int i = 0; i < ncands; i++)
        //     std::cout << "\t(" << neutevent->GetCand(i)->GetCandBlobID() << ") " << neutevent->GetCand(i)->GetCandRecoEDep();
        // std::cout << std::endl;
        // std::cout << "raw order      : ";
        ...
        if (m_neutevent->GetNNeutCands() > 0) {
            std::cout << "\tvtx:        " << "\t" << GetVec<double>("vtx").at(0) << "\t" << GetVec<double>("vtx").at(1) << "\t" << GetVec<double>("vtx").at(2) << std::endl;
            std::cout << "\tblobbegpos: " << "\t" << m_neutevent->GetNeutCand(0)->m_begposition.X() << "\t" << m_neutevent->GetNeutCand(0)->m_begposition.Y() << "\t" << m_neutevent->GetNeutCand(0)->m_begposition.Z() << std::endl;
        }
...
double CVUniverse::GetTrueTotNeutBlobEGeV() const {
    ...
    for (unsigned int i = 0; i < ntrueneutcands; i++){
        // edep += neutevent->GetTrueNeutCand(i)->GetCandRecoEDep();
        edep += m_neutevent->GetTrueNeutCand(i)->GetCandRecoEDep();
    }
    return edep * MeVGeV;
}

int CVUniverse::GetPlotNeutCandMCPID() const {
    if (GetNMADBlobs() < 1) {
        return -9999;
    }
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetTruthNeutEvent();
    ...
    if (nneutcands < 1)
        return -9999;
    // int pid = neutevent->GetNeutCand(0)->GetCandTruthPID();
    int pid = m_neutevent->GetNeutCand(0)->GetCandTruthPID();
...
int CVUniverse::GetPlotLeadingNeutCandTopMCPID() const {
    return GetPlotNeutCandTopMCPID(0);
}

int CVUniverse::GetPlotSecNeutCandTopMCPID() const {
    return GetPlotNeutCandTopMCPID(1);
}
...
double CVUniverse::GetNeutCandvtxZDist(int index) const {
    if (CVUniverse::GetNMADBlobs() < index + 1)
        return -99999.;
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    // if (neutevent->GetNNeutCands() < index + 1)
    if (m_neutevent->GetNNeutCands() < index + 1)
        return -99999.;
    // const std::unique_ptr<NeutronMultiplicity::NeutCand>& cand = neutevent->GetNeutCand(index);
    const std::unique_ptr<NeutronMultiplicity::NeutCand>& cand = m_neutevent->GetNeutCand(index);
    // return abs(cand->GetCandFlightPath().Z());
    return abs(cand->GetCandVtxZDist());
}

double CVUniverse::GetLeadingNeutCandvtxZDist() const {
    return GetNeutCandvtxZDist(0);
}
double CVUniverse::GetSecNeutCandvtxZDist() const {
    return GetNeutCandvtxZDist(1);
}
double CVUniverse::GetThirdNeutCandvtxZDist() const {
    return GetNeutCandvtxZDist(2);
}

double CVUniverse::GetNeutCandvtxTDist(int index) const {
    if (CVUniverse::GetNMADBlobs() < index + 1)
        return -99999.;
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    if (m_neutevent->GetNNeutCands() < index + 1)
        return -99999.;
    const std::unique_ptr<NeutronMultiplicity::NeutCand>& cand = m_neutevent->GetNeutCand(index);
    // return abs(cand->GetCandFlightPath().Rho());
    return abs(cand->GetCandVtxTDist());
}

double CVUniverse::GetLeadingNeutCandvtxTDist() const {
    return GetNeutCandvtxTDist(0);
}
double CVUniverse::GetSecNeutCandvtxTDist() const {
    return GetNeutCandvtxTDist(1);
}
double CVUniverse::GetThirdNeutCandvtxTDist() const {
    return GetNeutCandvtxTDist(2);
}

double CVUniverse::GetNeutCandvtxSphereDist(int index) const {
    if (CVUniverse::GetNMADBlobs() < index + 1)
        return -99999.;
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    if (m_neutevent->GetNNeutCands() < index + 1)
...
double CVUniverse::GetLeadingNeutCandvtxSphereDist() const {
    return GetNeutCandvtxSphereDist(0);
}
double CVUniverse::GetSecNeutCandvtxSphereDist() const {
    return GetNeutCandvtxSphereDist(1);
}
double CVUniverse::GetThirdNeutCandvtxSphereDist() const {
    return GetNeutCandvtxSphereDist(2);
}

double CVUniverse::GetNeutCandvtxBoxDist(int index) const {
    if (CVUniverse::GetNMADBlobs() < index + 1)
        return -99999.;
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    ...
    return std::max(abs(fp.Z()), abs(fp.Rho()));
}

double CVUniverse::GetLeadingNeutCandvtxBoxDist() const {
    return GetNeutCandvtxBoxDist(0);
}
double CVUniverse::GetSecNeutCandvtxBoxDist() const {
    return GetNeutCandvtxBoxDist(1);
}
double CVUniverse::GetThirdNeutCandvtxBoxDist() const {
    return GetNeutCandvtxBoxDist(2);
}

double CVUniverse::GetNeutCandEdepMeV(int index) const {
    if (CVUniverse::GetNMADBlobs() < index + 1)
        return -99999.;
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    if (m_neutevent->GetNNeutCands() < index + 1)
...
double CVUniverse::GetLeadingNeutCandEdepMeV() const {
    return GetNeutCandEdepMeV(0);
}
double CVUniverse::GetSecNeutCandEdepMeV() const {
    return GetNeutCandEdepMeV(1);
}
double CVUniverse::GetThirdNeutCandEdepMeV() const {
    return GetNeutCandEdepMeV(2);
}

double CVUniverse::GetNeutCandClusterMaxEMeV(int index) const {
    if (CVUniverse::GetNMADBlobs() < index + 1)
        return -99999.;
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    if (m_neutevent->GetNNeutCands() < index + 1)
        return -99999.;
    const std::unique_ptr<NeutronMultiplicity::NeutCand>& cand = m_neutevent->GetNeutCand(index);
    return cand->GetCandMaxClusterE();
}

double CVUniverse::GetLeadingNeutCandClusterMaxEMeV() const {
    return GetNeutCandClusterMaxEMeV(0);
}
double CVUniverse::GetSecNeutCandClusterMaxEMeV() const {
    return GetNeutCandClusterMaxEMeV(1);
}
double CVUniverse::GetThirdNeutCandClusterMaxEMeV() const {
    return GetNeutCandClusterMaxEMeV(2);
}

double CVUniverse::GetNeutCandMuonDist(int index) const {
    if (CVUniverse::GetNMADBlobs() < index + 1)
        return -99999.;
    // std::unique_ptr<NeutronMultiplicity::NeutEvent> neutevent = CVUniverse::GetNeutEvent();
    if (m_neutevent->GetNNeutCands() < index + 1)
    ...
    ROOT::Math::XYZVector pmu(GetMuon4V().X(), GetMuon4V().Y(), GetMuon4V().Z());
    ROOT::Math::XYZVector fp = cand->GetCandFlightPath();
    double angle = ROOT::Math::VectorUtil::Angle(pmu, fp);
    return abs(fp.R() * std::sin(angle));
}

double CVUniverse::GetLeadingNeutCandMuonDist() const {
    return GetNeutCandMuonDist(0);
}
double CVUniverse::GetSecNeutCandMuonDist() const {
    return GetNeutCandMuonDist(1);
}
double CVUniverse::GetThirdNeutCandMuonDist() const {
    return GetNeutCandMuonDist(2);
}
...
double CVUniverse::GetNeutCandMuonAngle(int index) const {
    ...
    ROOT::Math::XYZVector pmu(GetMuon4V().X(), GetMuon4V().Y(), GetMuon4V().Z());
    ROOT::Math::XYZVector fp = cand->GetCandFlightPath();
    return ROOT::Math::VectorUtil::Angle(pmu, fp) * 180. / M_PI;
}

double CVUniverse::GetLeadingNeutCandMuonAngle() const {
    return GetNeutCandMuonAngle(0);
}
double CVUniverse::GetSecNeutCandMuonAngle() const {
    return GetNeutCandMuonAngle(1);
}
...
double CVUniverse::GetLeadingNeutCandMuonCosTheta() const {
    return GetNeutCandMuonCosTheta(0);
}
...
double CVUniverse::GetLeadingNeutCandTrackEndDist() const {
    return GetNeutCandTrackEndDist(0);
}

double CVUniverse::GetSecNeutCandTrackEndDist() const {
    return GetNeutCandTrackEndDist(1);
}
...
double CVUniverse::GetNeutCandAngleToParent(int index) const {
    ...
    if (m_neutevent->GetNNeutCands() < index + 1)
        return -99999.;
    const std::unique_ptr<NeutronMultiplicity::NeutCand>& cand = m_neutevent->GetNeutCand(index);
    return cand->GetCandTruthAngleFromParent();
}
...
int CVUniverse::GetTruthHasSinglePositivePion() const {
    ...
    if (genie_n_pos_pion == 1) return 1;
    return 0;  // i.e. if genie_n_pos_pion == 0
}

int CVUniverse::GetTruePositivePionCount() const {
    int genie_n_pos_pion = 0;
    std::vector<int> mc_FSPartPDG = GetVecInt("mc_FSPartPDG");
    int mc_nFSPart = GetInt("mc_nFSPart");

    for (int i = 0; i < mc_nFSPart; i++)
        if (mc_FSPartPDG[i] == 211) genie_n_pos_pion++;

    return genie_n_pos_pion;
}
...
int CVUniverse::GetTruthHasSingleNegativePion() const {
    ...
    if (genie_n_neg_pion == 1) return 1;
    return 0;  // i.e. if genie_n_neg_pion == 0
}

int CVUniverse::GetTrueNegativePionCount() const {
    int genie_n_neg_pion = 0;
    std::vector<int> mc_FSPartPDG = GetVecInt("mc_FSPartPDG");
    int mc_nFSPart = GetInt("mc_nFSPart");
