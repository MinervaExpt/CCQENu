...
const std::string do_fractional_uncertainty_str = do_fractional_uncertainty ? std::string("Frac") : std::string("Abs");
const std::string do_cov_area_norm_str = do_cov_area_norm ? std::string("CovAreaNorm") : std::string("POT");
...
void PlotCVAndError(TCanvas& cE, PlotUtils::MnvH1D* idatahist, PlotUtils::MnvH1D* ihist, std::string label, bool cov_area = do_cov_area_norm, int logscale = 0, bool binwid = true) {
    ...
    if (!ihist || !idatahist) {
        std::cout << " No datahist " << ihist << " or mchist " << ihist << " " << label << std::endl;
        return;
    }
