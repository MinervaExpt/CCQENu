...
std::vector<CCQENu::VariableHyperDFromConfig *> GetHyperDVariablesVecFromConfig(std::vector<std::string> varsHD, std::vector<CCQENu::VariableFromConfig *> variablesvec, const std::vector<std::string> tags, const NuConfig configraw)
{
  ...
  std::vector<std::string> keys = config.GetKeys();
  for (auto key : keys)
  { // keys here is the list labelled "HyperD" in the Variables json file so you can define them all

    bool found = false; // This is a bool to check that the variable you configured in your *Main* config are found
    NuConfig varconfig; // This is the config for one HyperD variable from your Variable config
    for (auto v : varsHD)
    { // varsHD is the list from the Main config
      if (v == key)
      { // Only work on the vars configured in Variable config that are selected in your Main config
        found = true;
