...
template <class UNIVERSE, class EVENT = detail::empty>
class IsNotSameFromConfigFunc: public Cut<UNIVERSE, EVENT>
{
	public:
		typedef std::function<double(const UNIVERSE&)> PointerToCVUniverseIntFunction;
		IsNotSameFromConfigFunc(const NuConfig& config, CVFunctions<UNIVERSE>& functions): Cut<UNIVERSE, EVENT>(config.GetString("name")+ "!=" + std::to_string( config.GetInt("notequal"))), m_NoMatch(config.GetInt("notequal")), m_func(functions.GetRecoIntFunction(config.GetString("variable")))
		{
		}
#ifndef __GCCXML__
		virtual ~IsNotSameFromConfigFunc() = default;
#endif
	private:
		bool checkCut(const UNIVERSE& univ, EVENT& /*evt*/) const override
		{
			//std::cout << Cut<UNIVERSE, EVENT>::getName() << " " << m_func(univ) << " "  << m_NoMatch << std::endl;
			return m_func(univ) != m_NoMatch;
		}
		const PointerToCVUniverseIntFunction m_func;
		const int  m_NoMatch; //Maximum value of UNIVERSE::var that passes this Cut
};
...
template <class UNIVERSE>
class IsNotSameConstraintFromConfigFunc: public SignalConstraint<UNIVERSE>
{
	public:
		typedef std::function<double(const UNIVERSE&)> PointerToCVUniverseFunction;
		IsNotSameConstraintFromConfigFunc(const NuConfig& config, CVFunctions<UNIVERSE>& functions): SignalConstraint<UNIVERSE>(config.GetString("name")+ "!=" + (config.GetString("notequal"))), m_NoMatch(config.GetInt("notequal")),m_func(functions.GetTrueIntFunction(config.GetString("variable")))
		{
		}
#ifndef __GCCXML__
		virtual ~IsNotSameConstraintFromConfigFunc() = default;
#endif
	private:
		bool checkConstraint(const UNIVERSE& univ /*evt*/) const// override
		{
