...
public:
	...
	{
		if (fReader == NULL) {
			throw std::runtime_error("No XML file loaded. No model to compute with.");
		}
		else {
	...
	TMVA::Experimental::RTensor<float> Compute(TMVA::Experimental::RTensor<float> &x)
	{
		if (fReader == NULL) {
			throw std::runtime_error("No XML file loaded. No model to compute with.");
		}
			...
			TMVA::Experimental::RTensor<float> y({numEntries * numClasses});
			if (fAnalysisType == Internal::AnalysisType::Multiclass)
