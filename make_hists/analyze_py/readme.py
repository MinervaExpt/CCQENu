    ...
    if "h2D" in key:
        ...
        if key.find("migration"): 
            ...
            if (hist != 0) and hist != None: 
                ...
                response1D[sample][variable][thetype][category] = None
        ...
        #delete temp
            ...
            #print("could not read " , key )
            hists1D[sample][variable][thetype][category] = None
    ...
    for variable in variables:
        for thetype in ["reconstructed","reconstructed_scaled"]:
            ...
            for category in categoryMap.keys():
                 ...
                 if category in allconfigs["Fit"]["Backgrounds"]:
...
#     for samples in hists1D: 
#         sample = samples.first
#         print(" Sample: " , sample )
#         for variables in hists1D[sample]:   # only do this for a subset to save output time.

#             variable = variables.first
#             if not checktag(AnalyzeVariables, variable):
...
#             sys.exit = GetCrossSection(sample, variable, basename, hists1D[sample][variable], response1D[sample][variable], allconfigs, canvas1D, norm, POTScale, h_flux_dewidthed, unfold, num_iter, DEBUG, hasbkgsub, usetune, pdfname)
...
#             variable = variables.first
#             if "_resolution" in variable: continue
#             #if (variable.find("_resolution"): == std.string.npos): continue

#             for t in hists1D[sample][variable]: 
#                 resthetype = t.first
#                 print("resthetype " , resthetype )
#                 for c in hists1D[sample][variable][resthetype]: 
#                     category = c.first
#                     h = MnvH1D()
#                     h = hists1D[sample][variable][resthetype][category].Clone()
...
#     for samples in hists2D: 
#         sample = samples.first

#         for variables in hists2D[sample]:   # only do this for a subset to save output time.
#             variable = variables.first
