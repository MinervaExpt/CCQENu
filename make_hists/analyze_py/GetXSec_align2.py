...
#
def DoResponseUnfolding(basename, iresponse,
...
def GetCrossSection(sample, variable, basename,
    ...
    # If 2D, then set log scale for each axis if needed
        ...
        if ("Q2QE" in varpars[0]): logscale += 1
        if ("Q2QE" in varpars[1]): logscale += 2
    ...
    if (DEBUG): print("test pointers " , ibkgsubhist , " " , iseltruhist )
...
# HACK as the response seems to have gotten the wrong normalization
    ...
    print("Unfolding Vector Size: " , unsmearedVec.size() )
    ...
    fluxnorm = CheckFluxNorm(configs["varsFile"], variable)
