...
int main( int argc, char** argv )
	...
	// Cut optimisation
	Use["Cuts"]            = methodsConfig.GetBool("Cuts");
	Use["CutsD"]           = methodsConfig.GetBool("CutsD");
	Use["CutsPCA"]         = methodsConfig.GetBool("CutsPCA");
	Use["CutsGA"]          = methodsConfig.GetBool("CutsGA");
	Use["CutsSA"]          = methodsConfig.GetBool("CutsSA");
	//
	// 1-dimensional likelihood ("naive Bayes estimator")
	Use["Likelihood"]      = methodsConfig.GetBool("Likelihood");
	Use["LikelihoodD"]     = methodsConfig.GetBool("LikelihoodD");
	Use["LikelihoodPCA"]   = methodsConfig.GetBool("LikelihoodPCA");
	Use["LikelihoodKDE"]   = methodsConfig.GetBool("LikelihoodKDE");
	Use["LikelihoodMIX"]   = methodsConfig.GetBool("LikelihoodMIX");
	//
	// Mutidimensional likelihood and Nearest-Neighbour methods
	Use["PDERS"]           = methodsConfig.GetBool("PDERS");
	Use["PDERSD"]          = methodsConfig.GetBool("PDERSD");
	Use["PDERSPCA"]        = methodsConfig.GetBool("PDERSPCA");
	Use["PDEFoam"]         = methodsConfig.GetBool("PDEFoam");
	Use["PDEFoamBoost"]    = methodsConfig.GetBool("PDEFoamBoost");
	Use["KNN"]             = methodsConfig.GetBool("KNN");
	//
	// Linear Discriminant Analysis
	Use["LD"]              = methodsConfig.GetBool("LD");
	Use["Fisher"]          = methodsConfig.GetBool("Fisher");
	Use["FisherG"]         = methodsConfig.GetBool("FisherG");
	Use["BoostedFisher"]   = methodsConfig.GetBool("BoostedFisher");
	Use["HMatrix"]         = methodsConfig.GetBool("HMatrix");
	//
	// Function Discriminant analysis
	Use["FDA_GA"]          = methodsConfig.GetBool("FDA_GA");
	Use["FDA_SA"]          = methodsConfig.GetBool("FDA_SA");
	Use["FDA_MC"]          = methodsConfig.GetBool("FDA_MC");
	Use["FDA_MT"]          = methodsConfig.GetBool("FDA_MT");
	Use["FDA_GAMT"]        = methodsConfig.GetBool("FDA_GAMT");
	Use["FDA_MCMT"]        = methodsConfig.GetBool("FDA_MCMT");
	//
	// Neural Networks (all are feed-forward Multilayer Perceptrons)
	Use["MLP"]             = methodsConfig.GetBool("MLP");
	Use["MLPBFGS"]         = methodsConfig.GetBool("MLPBFGS");
	Use["MLPBNN"]          = methodsConfig.GetBool("MLPBNN");
	Use["CFMlpANN"]        = methodsConfig.GetBool("CFMlpANN");
	Use["TMlpANN"]         = methodsConfig.GetBool("TMlpANN");
#ifdef R__HAS_TMVAGPU
	Use["DNN_GPU"]         = methodsConfig.GetBool("DNN_GPU");
#else
	Use["DNN_GPU"]         = 0;
#endif

#ifdef R__HAS_TMVACPU
	Use["DNN_CPU"]         = methodsConfig.GetBool("DNN_CPU");
#else
	Use["DNN_CPU"]         = 0;
#endif
	//
	// Support Vector Machine
	Use["SVM"]             = methodsConfig.GetBool("SVM");
	//
	// Boosted Decision Trees
	Use["BDT"]             = methodsConfig.GetBool("BDT");
	Use["BDTG"]            = methodsConfig.GetBool("BDTG");
	Use["BDTB"]            = methodsConfig.GetBool("BDTB");
	Use["BDTD"]            = methodsConfig.GetBool("BDTD");
	Use["BDTF"]            = methodsConfig.GetBool("BDTF");
	//
	// Friedman's RuleFit method, ie, an optimised series of cuts ("rules")
	Use["RuleFit"]         = methodsConfig.GetBool("RuleFit");
	// ---------------------------------------------------------------
	...
	std::string bkgdT = config.GetString("BackgroundTree");
	...
	TCut mycutb = config.GetString("Cuts").c_str();
	...
	if (Use["BDTG"]) // Gradient Boost
		factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDTG",
		                     "!H:!V:NTrees=100:BoostType=Grad:Shrinkage=0.50:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=20:MaxDepth=3" );
