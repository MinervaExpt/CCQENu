...
```{r, message = F, warning = F, echo = F}
events <- read.csv("CSV_CCQENu_minervame1N.csv", sep=";")
```
...
events$Truth <- factor(events$Truth, levels = c("qelike","1chargedpion","1neutralpion","multipion","other"))
truth_colors = c("#0085ad","#af272f","#4c8c2b","#eaaa00","#522506")

diffplot <- function(evts, x, xbreaks = seq(0,1,0.2), title, bin_width, units = "", wgt = 1) {
  ggplot() +
    geom_histogram_pattern(evts,
             ...
             binwidth = bin_width, origin = 0, geom = "step") +
    stat_bin(evts,
             mapping = aes({{x}}, color = Truth, weight = {{wgt}}*weight/bin_width),
             binwidth = bin_width, origin = 0, geom = "step", position = position_stacknudge(x=-bin_width/2)) +
    ...
    scale_x_continuous(expand = c(0,0), limits = c(min(xbreaks),max(xbreaks)), breaks = xbreaks) +
...
propplot <- function(evts, x, xbreaks = seq(0,1,0.2), title, bin_width, units = "", wgt = 1) {
  ggplot() +
    geom_histogram_pattern(evts,
             mapping = aes({{x}}, pattern_fill = Truth, weight = {{wgt}}*weight/bin_width),
             pattern = "magick", fill = "white", pattern_type = "hs_diagcross", pattern_scale = 0.45,
             binwidth = bin_width, origin = 0, geom = "step", position = "fill") +
    stat_bin(evts,
             mapping = aes({{x}}, color = Truth, weight = {{wgt}}*weight/bin_width),
             binwidth = bin_width, origin = 0, geom = "step", position = position_fillnudge(x=-bin_width/2)) +
