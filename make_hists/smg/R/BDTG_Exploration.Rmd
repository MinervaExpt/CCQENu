...
CSV_CCQENu_minervame1N_MC_Inextinguishable_merged.csv
...
ggplot(events %>% filter(BlobCount <= 20)) +
  stat_bin(binwidth = 1, geom = "step", position = position_nudge(x = 0), mapping = aes(BlobCount, fill = Truth), size = 2) +
  stat_bin(binwidth = 1, geom = "step", color = "white", position = position_nudge(x = 0), mapping = aes(BlobCount, fill = Truth), size = 1) +
  stat_bin(binwidth = 1, geom = "step", position = position_nudge(x = 0), mapping = aes(BlobCount, color = Truth), size = 1, alpha = 0.8) +
  theme_bw() +
  scale_x_continuous(expand = c(0,0), limits = c(0,20)) +
  scale_y_continuous(expand = expansion(mult = c(0,0.05)))
```
```{r, message = F, warning = F, echo = F}
ggplot(events %>% filter(BlobCount >= 0)) +
...
ggplot(events %>% filter(BlobCount >= 0)) +
  stat_bin(binwidth = 100, geom = "step", position = position_nudge(x = 0), mapping = aes(BlobDistance0, fill = Truth), size = 2) +
  stat_bin(binwidth = 100, geom = "step", color = "white", position = position_nudge(x = 0), mapping = aes(BlobDistance0, fill = Truth), size = 1) +
  stat_bin(binwidth = 100, geom = "step", position = position_nudge(x = 0), mapping = aes(BlobDistance0, color = Truth), size = 1, alpha = 0.8) +
  theme_bw() +
  scale_x_continuous(expand = c(0,0), limits = c(0,3000)) +
  scale_y_continuous(expand = expansion(mult = c(0,0.05)))
```
...
ggplot(events %>%  
         filter(FittedMichelCount <= 5)) +
  stat_bin(binwidth = 1, position = position_stacknudge(x = .5), mapping = aes(BlobCount, fill = Truth)) +
  stat_bin(color = "black", geom = "step", binwidth = 1, position = position_stacknudge(x = 0), mapping = aes(BlobCount, fill = Truth)) + 
  theme_bw() + ggtitle("Fitted Michel Count") +
  scale_x_continuous(expand = c(0,0), limits = c(0,20)) +
  scale_y_continuous(expand = expansion(mult = c(0,0.05))) +
  facet_wrap(vars(FittedMichelCount), ncol = 3, scales = "free")
...
ggplot(events %>% filter(BlobCount >= 1)) +
  stat_bin(binwidth = 100, position = position_stacknudge(x = 50), mapping = aes(BlobDistance1, fill = Truth)) +
  stat_bin(color = "black", geom = "step", binwidth = 100, position = position_stacknudge(x = 0), mapping = aes(BlobDistance1, fill = Truth)) + 
  theme_bw() +
  scale_x_continuous(expand = c(0,0), limits = c(0,3000)) +
  scale_y_continuous(expand = expansion(mult = c(0,0.05)))

ggplot(events %>% filter(BlobCount >= 1)) +
  stat_bin(binwidth = 100, mapping = aes(BlobDistance1, fill = Truth, y = ..count../sum(..count..)), position = position_fillnudge(x = 50)) +
  stat_bin(color = "black", geom = "step", binwidth = 100, position = position_fillnudge(x = 0), mapping = aes(BlobDistance1, fill = Truth, y = ..count../sum(..count..))) + 
  theme_bw() +
  scale_x_continuous(expand = c(0,0), limits = c(0,3000)) +
  scale_y_continuous(expand = expansion(mult = c(0,0)))

ggplot(events %>% filter(BlobCount >= 1)) +
  stat_bin(binwidth = 100, geom = "step", position = position_nudge(x = 0), mapping = aes(BlobDistance1, fill = Truth), size = 2) +
  stat_bin(binwidth = 100, geom = "step", color = "white", position = position_nudge(x = 0), mapping = aes(BlobDistance1, fill = Truth), size = 1) +
  stat_bin(binwidth = 100, geom = "step", position = position_nudge(x = 0), mapping = aes(BlobDistance1, color = Truth), size = 1, alpha = 0.8) +
  theme_bw() +
  scale_x_continuous(expand = c(0,0), limits = c(0,3000)) +
  scale_y_continuous(expand = expansion(mult = c(0,0.05)))
```
...
evt_compare <- events %>%
  group_by(Truth) %>%
  count(name = "Ntruth")
evt_compare$wgtSums <- apply(as.matrix(events[,60:64]),2,sum)[c(2:5,1)]
...
pred_n <- tibble(Pred = map_chr(1:nrow(events),~labels[which.max(events[.x,60:64])])) %>%
  group_by(Pred) %>% count()
...
events$Pred <- map_chr(1:nrow(events),~bands[which.max(events[.x,c(24,20:22)])])
...
events %>%
  ...
  facet_wrap(vars(Pred),scales="free_y", ncol=1)
